...
func (m *MenderAuthManager) IsAuthorized(config conf.MenderConfig) bool {
	...
	if tenantToken=="" || config.TenantToken != tenantToken {
		log.Infof("    IsAuthorized config.TenantToken:%s != tenantToken:%s; err=%v.", config.TenantToken, tenantToken, err)
		m.UpdateAuthTenantToken(config.TenantToken)
		m.RemoveAuthToken()
		a,_ := m.AuthToken()
		t,_:=m.AuthTenantToken()
		log.Infof("    IsAuthorized currently: removed AuthToken, it is: '%s' reset tenant token: '%s', returning false;",a,t)
		return false
	}
